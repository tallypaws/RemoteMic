...
type Client = {
  ...
  socket: WebSocket;
  /*
    connecting: waiting for client to send role
    waiting: waiting for other room member to agree
    connected: both members agreed, ready to stream
     */
  state: "connecting" | "waiting" | "connected";
  role?: "drain" | "mic";
  accepted?: boolean;
  raw: boolean;
};

const rooms: Record<string, { drain?: Client; mic?: Client }> = {};
  ...
  ws.on("message", (message) => {
    try {
      if (client.raw) {
        ...
        if (otherClient.state !== "connected") return;
          ...
          case "role":
            {
              ...
              if (data.role !== "drain" && data.role !== "mic") return;
              const role = data.role as "drain" | "mic";
              ...
              if (rooms[room].drain && rooms[room].mic) {
                ...
                rooms[room].drain?.socket.send(
                  JSON.stringify({ type: "ready", secCode }),
                );
                rooms[room].mic?.socket.send(
                  JSON.stringify({ type: "ready", secCode }),
                );
              }
            }
          ...
          case "accept":
            {
              ...
              const otherClient = rooms[room][otherRole];
              if (!otherClient) return;
              if (otherClient.state !== "waiting") return;
